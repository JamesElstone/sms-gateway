...
usage() {
    cat <<EOF
Usage:
  sms-gateway-send.bash SERVER TOKEN DESTINATION MESSAGE [MESSAGE...]

Send an SMS through the SMS Gateway API from Bash.
...
Examples:
  bash ./examples/shells/sms-gateway-send.bash sms.example.net "\$TOKEN" +447700900000 Hello from Bash
  bash ./examples/shells/sms-gateway-send.bash http://sms.example.net/sms-gateway "\$TOKEN" +447700900000 "A longer message"
...
TEMP_DIR="${TEMP_DIR%/}"
[[ -n "$TEMP_DIR" ]] || TEMP_DIR="/"
RESPONSE_FILE="$(mktemp "$TEMP_DIR/sms-gateway-send-bash-response.XXXXXX")" || die "unable to create temporary response file"
...
for (( offset = 0; offset < message_length; offset += CHUNK_SIZE )); do
    part=$((part + 1))
